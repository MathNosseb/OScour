...
//on initialise le inb pour lire le port
...
char *readKey() {
    ...
    if (is_key_ready()) {
        ...
        //on récupère la valeur du scancode
        ...
        //je met des if pour éviter les erreurs car un swicth ne marche pas
        if (scancode == 0x39) {return " ";}
        if (scancode == 0x9C) {return "\n";}
        if (scancode == 0x10) {return "a";}
        if (scancode == 0x11) {return "z";}
        ...
        if (scancode == 0x27) {return "m";}
        if (scancode == 0x2C) {return "w";}
    ...
    //si il n'y a rien on renvoie un character vide
    return ""; 
}
